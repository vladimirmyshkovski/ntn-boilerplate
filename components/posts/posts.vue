...
export default {
  ...
  props: {
    postType: {
      type: String,
      default: 'blog',
      validator: (val) => ['blog', 'projects', 'glossary'].includes(val)
    },
    amount: {
  ...
  computed: {
    placeholderClasses() {
      ...
      return [...Array.from({ length: this.amount }, (_, i) => classes[i % classes.length])] // repeats classes after one another
    }
  },
  async mounted() {
    this.loading = true
  ...
  methods: {
    ...
    async fetchPosts(postType = this.postType, amount = this.amount, sortBy = this.sortBy) {
      return await this.$content(postType, this.$i18n.locale).sortBy(sortBy.key, sortBy.direction).limit(amount).fetch()
    }
  }
}
</script>
